...
from pymic.io.image_read_write import save_nd_array_as_image
from pymic.net_run.self_sup.util import volume_fusion, nonlienar_volume_fusion, augmented_volume_fusion
from pymic.net_run.self_sup.util import self_volume_fusion
...
class SelfSupVolumeFusion(SegmentationAgent):
    ...
    def training(self):
        ...
        for it in range(iter_valid):
            ...
            labels_prob = get_one_hot_seg(labels, class_num)
...
class SelfSupSelfVolumeFusion(SegmentationAgent):
    """
    Abstract class for self-supervised segmentation.
    ...
    """
    def __init__(self, config, stage = 'train'):
        super(SelfSupSelfVolumeFusion, self).__init__(config, stage)
 
    def training(self):
        class_num   = self.config['network']['class_num']
        iter_valid  = self.config['training']['iter_valid']
        fuse_ratio  = self.config['self_supervised_learning']['SelfVolumeFusion_fuse_ratio'.lower()]
        size_min    = self.config['self_supervised_learning']['SelfVolumeFusion_size_min'.lower()]
        size_max    = self.config['self_supervised_learning']['SelfVolumeFusion_size_max'.lower()]
...
class SelfSupNonLinearVolumeFusion(SegmentationAgent):
    """
    Abstract class for self-supervised segmentation.
    ...
    """
    def __init__(self, config, stage = 'train'):
        super(SelfSupNonLinearVolumeFusion, self).__init__(config, stage)
 
    def training(self):
        class_num   = 3
        iter_valid  = self.config['training']['iter_valid']
        block_range = self.config['self_supervised_learning']['NonLinearVolumeFusion_block_range'.lower()]
        size_min    = self.config['self_supervised_learning']['NonLinearVolumeFusion_size_min'.lower()]
        size_max    = self.config['self_supervised_learning']['NonLinearVolumeFusion_size_max'.lower()]
...
class SelfSupAugmentedVolumeFusion(SegmentationAgent):
    """
    Abstract class for self-supervised segmentation.
    ...
    """
    def __init__(self, config, stage = 'train'):
        super(SelfSupAugmentedVolumeFusion, self).__init__(config, stage)
 
    def training(self):
        class_num   = 5
        iter_valid  = self.config['training']['iter_valid']
        size_min    = self.config['self_supervised_learning']['AugmentedVolumeFusion_size_min'.lower()]
        size_max    = self.config['self_supervised_learning']['AugmentedVolumeFusion_size_max'.lower()]
