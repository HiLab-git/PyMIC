...
class ConvBlock(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, dropout_p = 0.0, dim = 3):
    ...
    def __init__(self, in_channels1, in_channels2, out_channels, dropout_p,
        ...
        if trilinear:
            ...
            self.up = nn.Sequential(
                ...
                nn.LeakyReLU(),
                nn.ConvTranspose3d(in_channels1, in_channels2, kernel_size=2, stride=2)
            )
...
class Encoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.proj   = nn.Conv3d(self.in_chns, self.ft_chns[0], kernel_size=3, padding=1)
        self.in_conv= ConvBlock(self.ft_chns[0], self.ft_chns[0], self.dropout[0])
...
class Decoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.out_conv = ConvLayer(self.ft_chns[0], self.n_class)
        if(self.mul_pred):
            self.out_conv1 = ConvLayer(self.ft_chns[1], self.n_class)
            self.out_conv2 = ConvLayer(self.ft_chns[2], self.n_class)
            self.out_conv3 = ConvLayer(self.ft_chns[3], self.n_class)
...
class HiFormer_v5(nn.Module):
    ...
    * Reference: Özgün Çiçek, Ahmed Abdulkadir, Soeren S. Lienkamp, Thomas Brox, Olaf Ronneberger:
    ...
    """
    def __init__(self, params):
        super(HiFormer_v5, self).__init__()
        self.params    = params
        self.in_chns   = self.params['in_chns']
        self.ft_chns   = self.params['feature_chns']
        self.dropout   = self.params['dropout']
