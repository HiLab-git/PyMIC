...
from pymic.net.net3d.unet3d import ConvBlock, DownBlock
# from nnFormer
...
# from nnFormer
class PatchEmbed_backup(nn.Module):
...
class PatchEmbed(nn.Module):
    """
    replace patch embed with conv layers"""
    def __init__(self, in_chns=1, ft_chns = [32, 64, 128], dropout = [0, 0, 0.2]):
        super().__init__()
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        print("number of layers in encoder", self.num_layers, depths)
        self.embed_dim = embed_dim
        ...
        self.patch_embed = PatchEmbed(in_chans, ft_chns=[embed_dim // 4, embed_dim //2, embed_dim])
...
class HiFormer_v1(nn.Module):
    def __init__(self, params):
        """
        replace the embedding layer with convolutional blocks
        """
        super(HiFormer_v1, self).__init__()
        # crop_size=[96,96,96],
        #         embedding_dim=192,
        #         input_channels=1, 
        #         num_classes=9, 
        ...
        crop_size      = params["input_size"]
        embed_dim      = params.get("embedding_dim", 192)
        input_channels = params["in_chns"]
        num_classes    = params["class_num"]
        self.conv_op   = nn.Conv3d
        depths         = params.get("depths", [2, 2, 2, 2]) 
        num_heads      = params.get("num_heads", [6, 12, 24, 48]) 
        patch_size     = params.get("patch_size", [4, 4, 4])     # for patch embedding
        window_size    = params.get("window_size", [4, 4, 8, 4]) # for swin transformer window
        self._deep_supervision = params.get("deep_supervision", False)
        ...
        self.model_down=Encoder(pretrain_img_size=crop_size,window_size=window_size,embed_dim=embed_dim,
            patch_size=patch_size,depths=depths,num_heads=num_heads,in_chans=input_channels, out_indices=range(len(depths)))
