...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.conv2 = ConvBlock(ft_chns[2], ft_chns[2], conv_dims[2], dropout[2])

        self.down1 = DownSample(ft_chns[0], ft_chns[1], down_dims[0], conv_dims[1])
        self.down2 = DownSample(ft_chns[1], ft_chns[2], down_dims[1], conv_dims[2])
        self.down3 = DownSample(ft_chns[2], ft_chns[3], down_dims[2], conv_dims[3])
        self.down4 = DownSample(ft_chns[3], ft_chns[4], down_dims[3], conv_dims[4])
        ...
        self.conv_t2 = ConvTransBlock(chns = ft_chns[2], 
        ...
        self.conv_t3 = ConvTransBlock(chns = ft_chns[3], 
        ...
        self.conv_t4 = ConvTransBlock(chns = ft_chns[4], 
                            input_resolution = r_t4,
                            window_size  = window_sizes[2],
                            depth        = depths[2],
                            num_head     = num_heads[2],
                            drop_rate    = dropout[4],
                            attn_drop_rate=dropout[4]
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.up3 = UpCatBlock(ft_chns[2], ft_chns[3], down_dims[2], conv_dims[2]) 
        self.up4 = UpCatBlock(ft_chns[3], ft_chns[4], down_dims[3], conv_dims[3]) 
        ...
        self.conv0 = ConvBlock(ft_chns[0], ft_chns[0], conv_dims[0], dropout[0])
        self.conv1 = ConvBlock(ft_chns[1], ft_chns[1], conv_dims[1], dropout[1])
        self.conv2 = ConvTransBlock(chns = ft_chns[2], 
                            input_resolution = r_t2,
                            window_size  = window_sizes[0],
                            depth        = depths[0],
                            num_head     = num_heads[0],
        ...
        self.conv3 = ConvTransBlock(chns = ft_chns[3], 
                    input_resolution = r_t3,
                    window_size  = window_sizes[1],
                    depth        = depths[1],
                    num_head     = num_heads[1],
        ...
        if(self.mul_pred):
            ...
            self.out_conv2 = ConvLayer(ft_chns[2], class_num)
            self.out_conv3 = ConvLayer(ft_chns[3], class_num)

    def forward(self, x):
        x0, x1, x2, x3, x4 = x 

        x_d3 = self.conv3(self.up4(x3, x4))
        x_d2 = self.conv2(self.up3(x2, x_d3))
        x_d1 = self.conv1(self.up2(x1, x_d2))
...
class HiFormer_v4(nn.Module):
    def __init__(self, params):
        """
        replace the embedding layer with convolutional blocks
        """
        super(HiFormer_v4, self).__init__()       
        in_chns     = params["in_chns"]
        class_num   = params["class_num"]
        input_size  = params["input_size"]
        ft_chns     = params.get("feature_chns", [32, 64, 128, 256, 512])
        down_dims   = params.get("down_dims", [3, 3, 3, 3, 3])
        conv_dims   = params.get("conv_dims", [3, 3, 3, 3, 3])
